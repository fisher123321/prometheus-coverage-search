...
void CoverageSearchManager::init(ros::NodeHandle &nh) {
    ...
    uav_cmd_pub_ = nh.advertise<prometheus_msgs::UAVCommand>(
        uav_name_ + "/prometheus/command", 1);
...
void CoverageSearchManager::updateFrontiers() {
    ...
    if (ms > 120.0) {
        ROS_WARN_THROTTLE(2.0, "[CoverageSearch] frontier update took %.1f ms, frontiers=%d",
    ...
    case EXEC_STATE::EXPLORING: {
        ...
        if (!selected_frontier) {
            ...
            if (current_frontiers > 0) {
                ...
                Eigen::Vector3d nearest_frontier = uav_pos_;
                for (auto &avg : frontier_avgs) {
                    double dist = (avg.head<2>() - uav_pos_.head<2>()).norm();
                    if (dist < best_dist && dist > 1e-3) {
                        best_dist = dist;
                        nearest_frontier = avg;
...
void CoverageSearchManager::publishCoverageStatus() {
    ...
    if (++print_cnt >= 4) {
        print_cnt = 0;
        cout << GREEN << "[CoverageSearch] State: " << exec_state_
