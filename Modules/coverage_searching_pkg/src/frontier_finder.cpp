...
void FrontierFinder::searchFrontiers(const Eigen::Vector3d &cur_pos) {
    ...
    // FALCON-style incremental removal: only clusters overlapping the updated AABB
    // and whose frontier cells are no longer valid are removed.
        ...
        for (int x = min_id(0); x <= max_id(0); x++) {
            for (int y = min_id(1); y <= max_id(1); y++) {
                for (int z = z_min; z <= z_max; z++) {
                    ...
                    std::vector<Eigen::Vector3d> cluster_cells;
                    expandFrontier(seed, cluster_cells);
                    if ((int)cluster_cells.size() >= cluster_min_ &&
