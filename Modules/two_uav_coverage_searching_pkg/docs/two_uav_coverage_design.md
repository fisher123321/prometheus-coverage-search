...
协调器仍只通过原有 `SwarmTask` lease 决定簇归属，不携带或广播路线顺序。执行端仅将**已确认且未过期的本机
lease**加入候选。本机传感器新发现或更新出的簇会绕过竞价：管理器立即以原有 `SwarmFrontier` reservation
字段上报，协调器随即生成并发布一条完全相同结构的本机 `SwarmTask`，租约为 8 秒；它因此与竞价获胜任务
使用同一消息、有效性检查和 RViz owner 标签。

每架机从自身最近的至多 5 个有效租约簇构建开放 ATSP。当前飞行状态作为虚拟起点：首边使用真实位置、速度、
偏航；其余边假定上一视点已静止，使用 `yaw_i -> yaw_j`。每条有向边复用 `Astar2D` 路径长度和既有
`computeFrontierCost()` 同类的时间瓶颈模型，因此矩阵会因起始速度、偏航和运动方向而非对称。任务上限为 5，
使用内置 Held–Karp 求精确开放路径，不引入 FUEL 的全局 LKH 文件和进程竞争。

所得顺序保存为“当前目标 + 剩余 tour”，并随既有 rolling handoff 一起交接。执行当前簇时若传感器发现本机新簇，
当前段保持不被打断，但旧 tour 的剩余段和已准备 handoff 立即作废；新簇被固定为下一轮 tour 的第一个任务，避免
刚看完 A 就沿用旧顺序飞往 B、错过 A 前方新出现的 AA。准备下一个目标前会复核所有剩余
视点的有效 lease、可见性、栅格可通行性和 ESDF 安全距离；任一项失效，或下一段 A* / 轨迹生成失败时，整个
剩余 tour 被丢弃并基于最新地图重算，而不是跳过失效节点。

当前 tour 的最后一个视点执行期间，后台从该段轨迹的终端 P/V/A/yaw 状态重解下一轮 ATSP，并完成到新首视点的
单段 NLOpt B 样条优化；交接固定在旧轨迹终点，预规划不会提前切走最后一个视点任务。

局部精修借鉴 FUEL 的两层结构：tour 前 3 个簇各保留前 3 个可见候选视点，以分层最短路选出连续的视点与偏航；
之后的簇仍使用其最佳视点。这些参数由 launch 中的 `coverage_search/atsp_*` 参数显式配置。
