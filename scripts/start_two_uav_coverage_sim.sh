...
if [[ "$capture_logs" == true ]]; then
  ...
  tracking_uav1_log="$log_dir/${run_id}_uav${uav1_id}_tracking.csv"
  tracking_uav2_log="$log_dir/${run_id}_uav${uav2_id}_tracking.csv"
  touch "$info_log" "$warn_log" "$tracking_uav1_log" "$tracking_uav2_log"
  echo "Normal log: $info_log"
  echo "Warning log: $warn_log"
  echo "Tracking UAV${uav1_id}: $tracking_uav1_log"
  echo "Tracking UAV${uav2_id}: $tracking_uav2_log"
  export TWO_UAV_TRACKING_LOG_PREFIX="$log_dir/$run_id"
  exec 3>>"$info_log"
  exec 4>>"$warn_log"
  set +e
...
package_bin="$workspace/devel/lib/prometheus_two_uav_coverage_search"
for executable in two_uav_coverage_search_node two_uav_coordinator two_uav_swarm_bridge; do
  [[ -x "$package_bin/$executable" ]] || {
    echo "Missing coverage executable: $package_bin/$executable" >&2
    echo "Build all message consumers first: cmake --build $workspace/build/two_uav_coverage_search -j1" >&2
    exit 1
  }
done
